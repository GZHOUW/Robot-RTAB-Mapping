...
double inverseSensorModel(double x, double y, double theta, double xi, double yi, double sensorData[]){
    ...
    for (int i = 0; i < 8; i++) { // sense the environment in 8 directions
        if (i == 0) {
    ...
    //Evaluate the three cases: unknown, occupied, and free --> l0 = 0, locc = 0.4, lfree = -0.4
    if (r > min((double)Zmax, Zk + alpha / 2) || fabs(phi - thetaK) > beta / 2 || Zk > Zmax || Zk < Zmin) { // outide the cone
        return l0;
    }
    else if (Zk < Zmax && fabs(r - Zk) < alpha / 2) { // end of the cone
        return locc;
    }
    else if (r <= Zk) { // inside the cone
        return lfree;
    }
}
    ...
    for (int x = 0; x < mapWidth / gridWidth; x++) {
        for (int y = 0; y < mapHeight / gridHeight; y++) {
            ...
            if (sqrt(pow(xi - Robotx, 2) + pow(yi - Roboty, 2)) <= Zmax) {
                // if inverse sensing detects occupied, increase belief 'l', if free, decrease
